...
class UserApi(CrudApi):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(model, schema, insert_schema, update_schema, *args, **kwargs)
        self.add_api_route(
            "/",
            self.get,
            methods=["GET"],
    ...
    def insert_privileged(
        self,
        insert_schema: UserInsertAdmin,
        session: Session = Depends(get_session),
...
class ProductFileApi(CrudApi):
    def __init__(
        ...
        update_schema: ProductFileUpdate = MakeOptionalPydantic.make_partial_model(
            ProductFileUpdate
        ),
        *args,
        **kwargs,
    ...
    async def insert(
        ...
        try:
            ...
            for insertion in files:
                ...
                result = await self.service.create_product_image_url(
                    insertion, filename, product_id
                )
                ...
                inserted_files.append(
                    self.crud.insert_item(insert_schema, session).model_dump()
                )
    ...
    async def delete(self, id: int, session: Session = Depends(get_session)):
        try:
            url_deleted = self.service.delete_product_url(id, session)
            file_deleted = super().delete(id, session=session)
            if file_deleted == []:
                return Response(
                    content="No product file found to delete ", status_code=200
                )
            return Response(
                content=dumps(
                    {"bucket_deleted": url_deleted, "db_deleted": file_deleted}
                ),
                status_code=200,
            )
        except Exception as exp:
...
class ProductApi(CrudApi):
    def __init__(
        ...
        update_schema: ProductUpdate = MakeOptionalPydantic.make_partial_model(
            ProductUpdate
        ),
        *args,
        **kwargs,
    ...
    def get(
        ...
        try:
            ...
            if results == []:
                return Response(content="No product found", status_code=200)
    ...
    def insert(
        self, insert_schema: ProductInsert, session: Session = Depends(get_session)
    ):
        try:
            ...
            logger.error(f"error at insert {self.__class__.__name__} {exp}")
            raise HTTPException(
                status_code=500, detail=f"Error at get insert product {exp}"
            )
    ...
    def update(
        ...
        try:
            ...
            logger.error(f"error at update {self.__class__.__name__} {exp}")
            raise HTTPException(
                status_code=500, detail=f"Error at get update product {exp}"
            )

    def delete(self, id: int = None, session: Session = Depends(get_session)):
        try:
            url_deleted = self.service.delete_product_urls(id, session)
            product_deleted = super().delete(id, session)
            if product_deleted == []:
                return Response(content="No product found to delete ", status_code=200)
            return Response(
                content=dumps(
                    {"bucket_deleted": url_deleted, "db_deleted": product_deleted}
                ),
                status_code=200,
            )
...
class PaymentApi(CrudApi):
    def __init__(
        ...
        update_schema: PaymentUpdate = MakeOptionalPydantic.make_partial_model(
            PaymentUpdate
        ),
        *args,
        **kwargs,
    ...
    def insert(
        self, insert_schema: PaymentInsert, session: Session = Depends(get_session)
    ):
        try:
...
class SaleApi(CrudApi):
    ...
    def insert(
        self, insert_schema: SaleInsert, session: Session = Depends(get_session)
    ):
...
class AddressApi(CrudApi):
    def __init__(
        ...
        update_schema: AddressUpdate = MakeOptionalPydantic.make_partial_model(
            AddressUpdate
        ),
        *args,
        **kwargs,
    ...
    def insert(
        self, insert_schema: AddressInsert, session: Session = Depends(get_session)
    ):
        try:
            return self.crud.insert_item(insert_schema, session)
        except Exception as exp:
