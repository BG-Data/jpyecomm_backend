...
class UserModel(DefaultModel):
    ...
    user_type = Column(
        String(30), nullable=False, default="comprador"
...
class AddressModel(DefaultModel):
    ...
    user_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, onupdate="CASCADE"
    )
...
class SaleModel(DefaultModel):
    ...
    product_id = Column(Integer, ForeignKey("products.id"), nullable=False)
    payment_method_id = Column(Integer, ForeignKey("payment_types.id"), nullable=False)
    buyer_id = Column(Integer, ForeignKey("users.id"), nullable=False)  # quem comprou
    ...
    sale_product = relationship("ProductModel", back_populates="product_sale")
