...
class CrudService(DatabaseSessions):
    ...
    def get_itens(self, kwargs: dict, session: Session) -> List[BaseModel]:
        ...
        try:
            ...
            if kwargs:
                ...
                item = item.filter(text(sql_filters.get("filter"))).params(
                    sql_filters.get("values")
                )
    ...
    def insert_item(self, insert_schema: BaseModel, session: Session) -> BaseModel:
        try:
    ...
    def update_item(self, item_id: int, update_schema: BaseModel, session: Session):
        try:
            ...
            to_update = update_schema.model_dump(exclude_unset=True, exclude_none=True)
            upd_result = item.update(to_update)
            self.update_session(session, item)
            logger.info(f"Rows updated: {upd_result}")
            logger.info(
                f"{update_schema.__repr_name__()} with id: {item.one().id} changed to {to_update}"
            )
    ...
    def delete_item(self, item_id: int, session: Session):
        try:
            ...
            return {
                "status": "deleted",
                "table": {"name": item.__tablename__, "id": item.id},
            }
        except Exception as exp:
...
class CrudApi(APIRouter):
    ...
    def delete(self, item_id: int, session: Session = Depends(get_session)):
        try:
